...
def run_generation():
    ...
    def load_trainer(model, tokenizer):
        """
        Helper function to re-initialize the trainer
        """
    ...
    for fold, (train_idx, eval_idx) in enumerate(splits):
        ...
        if fold == 0:
            trainer = load_trainer(model, tokenizer)
        ...
        if sft_args.do_eval:
            if not sft_args.do_train:
                # TODO: Refactor loading checkpoint model/tokenizer with the one from do_predict
            ...
            predictions_output = trainer.predict(eval_tokenized_dataset)
        ...
        if sft_args.do_predict:
            ...
            predict_result = trainer.predict(test_tokenized_dataset)
            predictions_df = pd.DataFrame(predict_result)
