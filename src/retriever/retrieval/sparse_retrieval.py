...
from src.retriever.score.ranking import (
    calculate_linear_score,
    calculate_reverse_rank_score,
    check_original_in_context,
)
...
class SparseRetrieval:
    ...
    def retrieve(
        ...
        topk: Optional[
            int
        ] = 1,  # Union은 주로 여러개의 typing 형이 가능할 때 주로 가용. Optinal은 1값이나 int형 값을 가질 수 있다는 의미.
    ) -> Union[
        Tuple[List, List], pd.DataFrame
    ]:  # 이 retrive의 결과로 튜플 형식의 (list, list)를 반환하거나 데이터프레임을 반환.
        """
        Arguments:
            query_or_dataset (Union[str, Dataset]):
        ...
        # query or dataset이 dataset이 아닐경우 -> 이는 쿼리가 한개가 아니라 여러개라는 의미.
            ...
            for idx, example in enumerate(
                ...
                tmp = {
                    ...
                    "retrieval_context": [
                        self.original_docs[pid] for pid in doc_indices[idx]
                    ],  # 현재 쿼리에 대한 가장 유사도가 높은 k개의 content들을 합침.
                }
    ...
    def get_relevant_doc(self, query: str, k: Optional[int] = 1) -> Tuple[List, List]:
        ...
        doc_score = result.squeeze()[sorted_result].tolist()[
            :k
    ...
    def get_relevant_doc_bulk(
        self, queries: List, k: Optional[int] = 5
    ) -> Tuple[List, List]:
        """
        Arguments:
            queries (List):
        ...
        assert np.sum(query_vecs) != 0, "query_vecs가 제대로 변환되지않음."

        print(query_vecs.shape, self.p_embedding.shape)
        # 유사도 계산

        result = (
            query_vecs @ self.p_embedding.T
        )  # 행렬 곱 연산 (질문수, 임베딩 차원) x (임베딩 차원, 문서수)
    ...
    def get_score(self, df):
        ...
        print("reverse rank retrieval:", round(df["rmm_score"].sum() / len(df)))
        print("linear retrieval:", round(df["linear_score"].sum() / len(df)))
    ...
    def compute_l2_distance(query_vec, passage_vec) -> np.ndarray:
        ...
        for i in tqdm(range(num_queries), desc="Computing L2 distances"):
            ...
            l2_distances[i] = cdist(
                query_vec[i : i + 1], passage_vec, metric="euclidean"
            ).flatten()
